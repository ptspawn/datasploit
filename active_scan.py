...
parser.add_option('-e', '--email_file', action="store", dest="emailfile", help="File containing list of Email ids",
                  default="spam")
parser.add_option('-s', '--subdomain_file', action="store", dest="subdomain_file",
                  help="File containing list of subdomains.", default="spam")
...
def run_active(filename, entity):
    ...
    if entity == "subdomains":
        ...
        print(
            colored(style.BOLD + "\n[+] Running Active Scan on " + str(len(subdomain_list)) + " subdomains" + style.END,
                    'green'))
        print("\n")
